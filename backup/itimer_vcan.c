...
int sock1,sock2; // Socket descriptor
...
// TODO: implement encoder/decoder functions for CAN messages for each state
// TODO: integrate ADC code to collect data
// TODO: integrate a second LCM package to avoid data collision from the async
// communication nature of the application
...
int main() 
{
  ...
  // Set interface name (e.g., "vcan0" for virtual CAN)
  ...
  // Create a socket
  sock2 = socket(PF_CAN, SOCK_RAW, CAN_RAW);
  if (sock2 == -1) {
      perror("Failed to create socket");
      return 1;
  }

  // Set interface name (e.g., "vcan1" for virtual CAN)
  strcpy(ifr2.ifr_name, CAN_INTERFACE_2);
  ...
  sendCANFrame(sock1, 0x123, data, 8);

  // Receive the message on sock2
  receiveCANFrame(sock2);
  ...
  data[6] = 0x77;
  data[7] = 0x88;
  sendCANFrame(sock2, 0x456, data, 8);
  // Receive the message on sock2
  receiveCANFrame(sock1);
...
void interval_expired(int id) // 1000 Hz timer 
{
  ...
  if(state == 1)
  {
    ...
    sendCANFrame(sock1, 0x123, data, 1);
    // READS CAN FRAME
    state = 0; // reset the state
    motor_data.position = sample_waveform*6.0f;
    motor_data.velocity = sample_waveform*60.0f;
    motor_data.iq = sample_waveform*30.0f;
    motor_data.vq = sample_waveform*20.0f;
    motor_data.torque = sample_waveform*1.0f;
  ...
  else if(state == 2)
  {
    ...
    data[0] = state;
    sendCANFrame(sock1, 0x123, data, 8);
    // TODO: READS CAN FRAME 
    state = 0; // reset the state
    motor_data.velocity = sample_waveform*0.0f;
    motor_data.iq = sample_waveform*0.0f;
    motor_data.vq = sample_waveform*0.0f;
    motor_data.torque = sample_waveform*0.0f;
    motor_data.cmd_id = state; // reset the state
    //FOC_motor_t_publish(lcm2,"MOTOR",&motor_data); // send signal to GUI to reset the flag
    FOC_motor_t_publish(lcm, "GUI", &motor_data);
  }
  else if(state == 3) // 100Hz
  {
    ...
    if(loop_count > CAN_FREQ)
    {
      ...
      memset(data, 0, sizeof(data)); // Initialize data to zero
      data[0] = state; // get 8 MSB
      data[1] = vq_int>>8; // get 8 MSB
      data[2] = vq_int&0xFF; // get 8 LSB
      sendCANFrame(sock1, 0x123, data, 8);
      // TODO: READS CAN MESSAGE
      motor_data.position = sample_waveform*vq_cmd/10.0f;
      motor_data.velocity = sample_waveform*vq_cmd*3.0f;
      motor_data.iq = vq_cmd;
      motor_data.vq = vq_cmd;
      motor_data.torque = sample_waveform/22.0f;
      motor_data.cmd_id = state; // reset the state
      //FOC_motor_t_publish(lcm2,"MOTOR",&motor_data); // send signal to GUI to reset the flag
      FOC_motor_t_publish(lcm, "GUI", &motor_data);
    }
  ...
  {    
    ...
    if(loop_count > CAN_FREQ)
    {
      ...
      // Sends CAN message 
      pos_cmd_int = float_to_uint16(pos_cmd,-10.0,10.0);
      kp_int = float_to_uint8(kp,0.0,0.1);
      kd_int = float_to_uint8(kd,0.0,0.1);
      memset(data, 0, sizeof(data)); // Initialize data to zero
      data[0] = state; // get 8 MSB
      data[1] = pos_cmd_int>>8; // get 8 MSB
      data[2] = pos_cmd_int&0xFF; // get 8 LSB
      data[3] = kp_int;
      data[4] = kd_int; 
      sendCANFrame(sock1, 0x123, data, 8);
      // TODO: READS CAN MESSAGE
      motor_data.position = pos_cmd;
      motor_data.velocity = pos_cmd;
      motor_data.iq = sample_waveform*30.0f;
      motor_data.vq = sample_waveform*22.0f;
      motor_data.torque = sample_waveform;
      motor_data.cmd_id = state; // reset the state
      //FOC_motor_t_publish(lcm2,"MOTOR",&motor_data); // send signal to GUI to reset the flag
      FOC_motor_t_publish(lcm, "GUI", &motor_data);
    }
  ...
  else if(state == 5) // 100Hz
  {
    ...
    if(loop_count > CAN_FREQ)
    {
      ...
      // Sends CAN message 
      vel_cmd_int = float_to_uint16(vel_cmd,-10.0,10.0);
      kp_int = float_to_uint8(kp,0.0,0.1);
      kd_int = float_to_uint8(kd,0.0,0.1);
      memset(data, 0, sizeof(data)); // Initialize data to zero
      data[0] = state; // get 8 MSB
      data[1] = vel_cmd_int>>8; // get 8 MSB
      data[2] = vel_cmd_int&0xFF; // get 8 LSB
      data[3] = kp_int;
      data[4] = kd_int;
      sendCANFrame(sock1, 0x123, data, 8);
      // TODO: READS CAN MESSAGE
      motor_data.position = sample_waveform*6.0f;
      motor_data.velocity = vel_cmd;
      motor_data.iq = sample_waveform*30.0f;
      motor_data.vq = sample_waveform*22.0f;
      motor_data.torque = sample_waveform;
      motor_data.cmd_id = state; // reset the state
  ...
  else if(state == 6) // dyno test
  {
    ...
    if(test_count > TIMER_FREQ) // 1Hz 
    {
      ...
      if(num_test_points > NUM_SAMPLES) // sweep across 10 cmd voltages
      {
        ...
        sendCANFrame(sock1, 0x123, data, 8);
        // TODO: READS CAN MESSAGE
        motor_data.position = sample_waveform*0.0f;
        motor_data.velocity = sample_waveform*0.0f;
        motor_data.iq = vq_cmd;
        motor_data.vq = vq_cmd;
        motor_data.torque = sample_waveform*0.0f;
